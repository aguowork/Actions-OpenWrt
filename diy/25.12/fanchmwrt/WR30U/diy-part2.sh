...
# AP 名字和中继上游统一在 files/etc/uci-defaults/WR30U 里设。
...
# sed -i 's/#zjwifi\*\/[^ ]* \*/\*\/59 \*/' files/etc/crontabs/root
...
sed -i "s/option index '.*'/option index '1'/g" feeds/nas/ddnsto/files/ddnsto.config
...
# 别照抄 24.10 的 `find ! -name 'WR30U' -exec rm`：这个目录里还有一个跟设备无关的
